...
async def on_message(message):
    ...
    if content_lower == "!bngkbeo":
        ...
        try:
            ...
            # 1. Update weight + 1
            update_sql = "UPDATE nhibeo SET nhibeo_weight = nhibeo_weight + 1 WHERE id = 2"
            cursor.execute(update_sql)
            connection.commit()

            # 2. Get new weight
            select_sql = "SELECT nhibeo_weight FROM nhibeo WHERE id = 2"
            cursor.execute(select_sql)
            result = cursor.fetchone()
            print(result)
            if result:
                new_weight = result[0]
                await message.channel.send(f"Số cân nặng hiện tại của Bảo Ngọc: **{new_weight}kg**")
    ...
    # ======================================================
    # COMMAND 6: !bngksieubeo (NEW)
    # Adds random weight 0-100 with weighted probability
    # ======================================================
    if content_lower == "!bngksieubeo":
        if message.channel.id != TARGET_CHANNEL_IDS[3]: return # Restrict channel
        # 1. Logic to pick random number with rarity
        # We roll a dice from 0 to 100 to determine "Luck Tier"
        chance = random.randint(0, 100)
        ...
        if chance <= 60:
            ...
            added_weight = random.randint(1, 32)
            food_item = random.choice(SMALL_FOOD)
            template = "Bảo Ngọc đã ăn **{food}** và tăng **{n}** cân! Số cân hiện tại của Bảo Ngọc: **{total}kg**"
        elif chance <= 95:
            # MEDIUM TIER
            added_weight = random.randint(33, 67)
            food_item = random.choice(MEDIUM_FOOD)
            template = "Khá béo! Bảo Ngọc đã húp trọn **{food}** và tăng **{n}** cân! Số cân hiện tại của Bảo Ngọc: **{total}kg**"
        else:
            # BIG TIER (Jackpot)
            added_weight = random.randint(68, 100)
            food_item = random.choice(BIG_FOOD)
            if food_item == "NGUYÊN 1 CON CẠP":
                template = "🚨 **NỔ HŨ THẾ GIỚI**! Bảo Ngọc đã chén sạch **{food}** và tăng **{n}** cân! Số cân hiện tại của Bảo Ngọc: **{total}kg**"
            else:
                template = "🚨 **NỔ HŨ**! Bảo Ngọc đã chén sạch **{food}** và tăng **{n}** cân! Số cân hiện tại của Bảo Ngọc: **{total}kg**"
        ...
        try:
            ...
            # Update DB
            update_sql = "UPDATE nhibeo SET nhibeo_weight = nhibeo_weight + %s WHERE id = 2"
            cursor.execute(update_sql, (added_weight,))
            connection.commit()
